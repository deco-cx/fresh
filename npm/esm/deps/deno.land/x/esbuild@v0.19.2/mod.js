...
async function installFromNPM(name, subpath) {
    ...
    try {
        await dntShim.Deno.stat(finalPath);
        return finalPath;
    }
    catch (e) {
    }
    const npmRegistry = dntShim.Deno.env.get("NPM_CONFIG_REGISTRY") || "https://registry.npmjs.org";
    ...
    await dntShim.Deno.mkdir(finalDir, {
        recursive: true,
        mode: 448
        // https://specifications.freedesktop.org/basedir-spec/basedir-spec-latest.html
    });
    await dntShim.Deno.writeFile(finalPath, executable, { mode: 493 });
    return finalPath;
}
function getCachePath(name) {
    ...
    switch (dntShim.Deno.build.os) {
        case "darwin":
            baseDir = dntShim.Deno.env.get("HOME");
            if (baseDir)
                baseDir += "/Library/Caches";
            break;
        case "windows":
            baseDir = dntShim.Deno.env.get("LOCALAPPDATA");
            if (!baseDir) {
                baseDir = dntShim.Deno.env.get("USERPROFILE");
                if (baseDir)
            ...
            break;
        case "linux":
            const xdg = dntShim.Deno.env.get("XDG_CACHE_HOME");
            if (xdg && xdg[0] === "/")
                baseDir = xdg;
            break;
    }
...
async function install() {
    const overridePath = dntShim.Deno.env.get("ESBUILD_BINARY_PATH");
    if (overridePath)
        return overridePath;
    const platformKey = dntShim.Deno.build.target;
...
var defaultWD = dntShim.Deno.cwd();
...
var ensureServiceIsRunning = () => {
    if (!longLivedService) {
        longLivedService = (async () => {
            ...
            const isTTY = dntShim.Deno.isatty(dntShim.Deno.stderr.rid);
            const child = dntShim.Deno.run({
            ...
            const readMoreStdout = () => child.stdout.read(stdoutBuffer).then((n) => {
                ...
                if (e instanceof dntShim.Deno.errors.Interrupted || e instanceof dntShim.Deno.errors.BadResource) {
                    afterClose(e);
                }
                else {
                    throw e;
                ...
                transform: (input, options) => new Promise((resolve, reject) => service.transform({
                    ...
                    fs: {
                        readFile(tempFile, callback) {
                            dntShim.Deno.readFile(tempFile).then((bytes) => {
                                let text = new TextDecoder().decode(bytes);
                                try {
                                    dntShim.Deno.remove(tempFile);
                                }
                                catch (e) {
                                }
                                callback(null, text);
                        ...
                        writeFile(contents, callback) {
                            dntShim.Deno.makeTempFile().then((tempFile) => dntShim.Deno.writeFile(tempFile, typeof contents === "string" ? new TextEncoder().encode(contents) : contents).then(() => callback(tempFile), () => callback(null)), () => callback(null));
                        }
                    },
                    callback: (err, res) => err ? reject(err) : resolve(res)
                })),
...
if ((import.meta.url === ("file:///" + process.argv[1].replace(/\\/g, "/")).replace(/\/{3,}/, "///"))) {
    dntShim.Deno.run({
        cmd: [await install()].concat(dntShim.Deno.args),
        cwd: defaultWD,
        stdin: "inherit",
        stdout: "inherit",
        stderr: "inherit"
