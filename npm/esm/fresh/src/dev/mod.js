...
export function ensureMinDenoVersion() {
    ...
    if (!gte(dntShim.Deno.version.deno, MIN_DENO_VERSION)) {
        let message = `Deno version ${MIN_DENO_VERSION} or higher is required. Please update Deno.\n\n`;
        if (dntShim.Deno.execPath().includes("homebrew")) {
            message +=
...
async function collectDir(dir, callback) {
...
${routes.map((file, i) => `import * as $${i} from "${toImportSpecifier(file)}";`).join("\n")}
${islands.map((file, i) => `import * as $$${i} from "${toImportSpecifier(file)}";`)
        .join("\n")}

const manifest = {
  routes: {
    ${routes.map((file, i) => `${JSON.stringify(`${toImportSpecifier(file)}`)}: $${i},`)
        .join("\n    ")}
  },
  islands: {
    ${islands.map((file, i) => `${JSON.stringify(`${toImportSpecifier(file)}`)}: $$${i},`)
        .join("\n    ")}
  },
...
`;
    const proc = new dntShim.Deno.Command(dntShim.Deno.execPath(), {
        args: ["fmt", "-"],
        stdin: "piped",
        stdout: "piped",
        stderr: "null",
    ...
    await dntShim.Deno.writeTextFile(manifestPath, manifestStr);
