...
function getHomeDir() {
    switch (dntShim.Deno.build.os) {
        ...
        case "windows":
            return dntShim.Deno.env.get("LOCALAPPDATA") ?? null;
    }
    return null;
}
...
export async function updateCheck(interval, getCacheDir = getFreshCacheDir, getLatestVersion = fetchLatestVersion, getCurrentVersion = readCurrentVersion) {
    ...
    if (dntShim.Deno.env.get("CI") === "true" ||
        dntShim.Deno.env.get("FRESH_NO_UPDATE_CHECK") === "true" ||
    ...
    try {
        await dntShim.Deno.mkdir(home, { recursive: true });
    }
    catch (err) {
    ...
    try {
        const text = await dntShim.Deno.readTextFile(filePath);
    ...
    if (semver.lt(currentVersion, latestVersion)) {
        ...
        console.log(colors.dim(`    To upgrade, run: `) + colors.dim(`deno run -A -r https://fresh.deno.dev/update .`));
        console.log();
    }
    const raw = JSON.stringify(checkFile, null, 2);
    await dntShim.Deno.writeTextFile(filePath, raw);
}
