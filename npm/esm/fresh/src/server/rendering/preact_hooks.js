...
options.vnode = (vnode) => {
    ...
    else if (current && typeof vnode.type === "function" && vnode.type !== Fragment &&
        ownerStack.length > 0) {
            ...
            patchIsland: if (island &&
                ...
                vnode.type = (props) => {
                    ...
                    if ("children" in props) {
                        ...
                        const markerText = `frsh-slot-${island.id}:${island.exportName}:${id}:children`;
                        // @ts-ignore nonono
                        props.children = wrapWithMarker(children, markerText);
                        slots.set(markerText, children);
                        children = props.children;
                        // deno-lint-ignore no-explicit-any
                        props.children = h(SlotTracker, { id: markerText }, children);
                    }
                    const child = h(originalType, props);
                    patched.add(child);
                    islandProps.push(props);
                    return wrapWithMarker(child, `frsh-${island.id}:${island.exportName}:${islandProps.length - 1}`);
                };
            }
        }
    }
