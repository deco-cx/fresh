...
export function renderHtml(state) {
    ...
    try {
        ...
        { value: state.csp }, h(HEAD_CONTEXT.Provider, {
...
export function renderOuterDocument(state, opts) {
    const { docHtml, docHead, renderedHtmlTag, docTitle, docBody, docHeadNodes, headVNodes, } = state;
    const page = h("html", docHtml ?? { lang: opts.lang }, h("head", docHead, !renderedHtmlTag ? h("meta", { charSet: "utf-8" }) : null, !renderedHtmlTag
        ? (h("meta", {
            name: "viewport",
            content: "width=device-width, initial-scale=1.0",
        }))
        : null, docTitle, docHeadNodes.map((node) => h(node.type, node.props)), opts.preloads.map((src) => h("link", { rel: "modulepreload", href: src })), opts.moduleScripts.map(([src, nonce]) => h("script", { src: src, nonce, type: "module" })), headVNodes), h("body", {
        ...docBody,
