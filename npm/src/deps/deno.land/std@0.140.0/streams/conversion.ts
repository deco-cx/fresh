...
function isCloser(value: unknown): value is dntShim.Deno.Closer {
...
export function readableStreamFromReader(
  reader: dntShim.Deno.Reader | (dntShim.Deno.Reader & dntShim.Deno.Closer),
 ...
 * ```
 */
export async function readAll(r: dntShim.Deno.Reader): Promise<Uint8Array> {
 ...
 * ```
 */
export function readAllSync(r: dntShim.Deno.ReaderSync): Uint8Array {
  const buf = new Buffer();
  buf.readFromSync(r);
  return buf.bytes();
}
 ...
 * ```
 */
export async function writeAll(w: dntShim.Deno.Writer, arr: Uint8Array) {
  let nwritten = 0;
 ...
 * ```
 */
export function writeAllSync(w: dntShim.Deno.WriterSync, arr: Uint8Array): void {
  let nwritten = 0;
  while (nwritten < arr.length) {
...
export async function copy(
  src: dntShim.Deno.Reader,
  dst: dntShim.Deno.Writer,
  options?: {
    bufSize?: number;
  },
): Promise<number> {
