...
export class Server {
  ...
  #handler: Handler;
  #closed = false;
  #listeners: Set<dntShim.Deno.Listener> = new Set();
  #acceptBackoffDelayAbortController = new AbortController();
  #httpConnections: Set<dntShim.Deno.HttpConn> = new Set();
  #onError: (error: unknown) => Response | Promise<Response>;
   ...
   */
  async serve(listener: dntShim.Deno.Listener) {
    if (this.#closed) {
   ...
   */
  async #respond(
    requestEvent: dntShim.Deno.RequestEvent,
    connInfo: ConnInfo,
  ) {
   ...
   * @param connInfo Information about the underlying connection.
   */
  async #serveHttp(httpConn: dntShim.Deno.HttpConn, connInfo: ConnInfo) {
    while (!this.#closed) {
      let requestEvent: dntShim.Deno.RequestEvent | null;
   ...
   * @param listener The listener to accept connections from.
   */
  async #accept(listener: dntShim.Deno.Listener) {
    let acceptBackoffDelay: number | undefined;

    while (!this.#closed) {
      let conn: dntShim.Deno.Conn;
      ...
      let httpConn: dntShim.Deno.HttpConn;
   ...
   */
  #closeHttpConn(httpConn: dntShim.Deno.HttpConn) {
   ...
   */
  #trackListener(listener: dntShim.Deno.Listener) {
   ...
   */
  #untrackListener(listener: dntShim.Deno.Listener) {
   ...
   */
  #trackHttpConnection(httpConn: dntShim.Deno.HttpConn) {
   ...
   */
  #untrackHttpConnection(httpConn: dntShim.Deno.HttpConn) {
...
export async function serveListener(
  listener: dntShim.Deno.Listener,
  handler: Handler,
...
export async function serve(
  ...
  port = (server.addrs[0] as dntShim.Deno.NetAddr).port;
