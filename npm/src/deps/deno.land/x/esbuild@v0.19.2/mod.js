        ...
        transform: (input, options) => new Promise((resolve, reject) => service.transform({
          ...
          fs: {
            readFile(tempFile, callback) {
              dntShim.Deno.readFile(tempFile).then(
                (bytes) => {
            ...
            writeFile(contents, callback) {
              dntShim.Deno.makeTempFile().then(
                (tempFile) => dntShim.Deno.writeFile(tempFile, typeof contents === "string" ? new TextEncoder().encode(contents) : contents).then(
                  () => callback(tempFile),
                  () => callback(null)
                ),
                () => callback(null)
