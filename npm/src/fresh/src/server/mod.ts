...
export async function start(routes: Manifest, opts: StartOptions = {}) {
  ...
  const portEnv = dntShim.Deno.env.get("PORT");
  ...
  if (opts.port) {
    ...
    for (let port = 8000; port < 8020; port++) {
      try {
        ...
        if (err instanceof dntShim.Deno.errors.AddrInUse) {
...
async function bootServer(handler: ServeHandler, opts: StartOptions) {
  ...
  if (typeof dntShim.Deno.serve === "function") {
    // @ts-ignore Ignore type error when type checking with Deno versions
    await dntShim.Deno.serve(opts, handler).finished;
  } else {
